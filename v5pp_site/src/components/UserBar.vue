<template>
    <div id="menubar" style="float:middle;font-weight: bold;">
        <el-menu :default-active="this.$router.path" class="el-menu-demo" mode="horizontal" @select="handleSelect">
            <el-menu-item index="1" class="list-group-item active" @click="toUserInfo">我 的 信 息</el-menu-item>
            <el-menu-item index="2" @click="toUserPhoneBook">组 员 联 系 方 式</el-menu-item>
            <el-submenu index="3">
                <template slot="title">我 的 经 费</template>
                <el-menu-item index="3-1" style="float:middle;font-weight: bold;">概 览 </el-menu-item>
                <el-menu-item index="3-2" style="float:middle;font-weight: bold;">经 费 申 请</el-menu-item>
                <el-menu-item index="3-3" style="float:middle;font-weight: bold;">发 票 上 传</el-menu-item>
                <el-menu-item index="3-4" style="float:middle;font-weight: bold;">收 款 确 认</el-menu-item>
            </el-submenu>
            <el-menu-item index="4">我 的 比 赛 日 程</el-menu-item>
            <el-menu-item index="5" disabled>管 理 账 目</el-menu-item>
            <el-menu-item index="6" disabled>管 理 招 新</el-menu-item>
            <el-menu-item index="6" disabled>管 理 团 队</el-menu-item>
            <el-menu-item index="7">管 理 账 号</el-menu-item>
            <div style="float:right;background-color:rgb(220, 230, 256);font-weight: bold;">
                <el-menu-item index="5" @click="toLogin">返 回 主 页 </el-menu-item>
            </div>
        </el-menu>
        <div class="line"></div>
    </div>
</template>

<script>
export default {
    data() {
        return {
        };
    },
    methods: {
        handleSelect(key, keyPath) {
            console.log(key, keyPath);
        },
        toLogin() {
            this.$router.push('/');
        },
        toUserInfo() {
            this.$router.push('/user/info');
        },
        toUserPhoneBook() {
            this.$router.push('/user/phonebook');
        }
    }
}
</script>